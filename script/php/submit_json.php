...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $text_original = trim($_POST['text_original']);
    $text_english = trim($_POST['text_english']);
    ...
    // Check duplicates (same pseudonym + same original text)
    $isDuplicate = false;
    foreach ($data as $entry) {
        if (
            $entry["pseudonym"] === $pseudonym &&
            $entry["text_original"] === $text_original
        ) {
            $isDuplicate = true;
            break;
        }
